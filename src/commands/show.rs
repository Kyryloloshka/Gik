...
pub fn show(storage: &Storage, target: &str) -> Result<()> {
    ...
    if parts.len() != 2 {
        return Err(GikError::Validation(
            "Expected format <hash-or-ref>:<path> or <hash-or-ref>".to_string(),
        ));
    }
    ...
    let mut commit_hash = if real_ref == "HEAD" {
        storage
            .commits()
            .get_current_head()?
            .ok_or_else(|| GikError::NotFound("HEAD is detached or missing".to_string()))?
    ...
    for part in path_parts.iter() {
        if let Some(compressed) = storage.objects().get_object(&current_hash)? {
            ...
            if obj_type != "tree" {
                return Err(GikError::Validation(format!(
                    "Expected tree, found {}",
                    obj_type
                )));
            }
...
fn show_commit_files(storage: &Storage, target: &str) -> Result<()> {
    ...
    let mut all_paths: Vec<String> = current_files
        .keys()
