...
fn test_checkout_basic() {
    ...
    assert!(
        !std::path::Path::new("b.txt").exists(),
        "b.txt should not exist in commit 1"
    );
...
fn test_checkout_by_bookmark() {
    ...
    crate::commands::checkout::checkout(storage, "feature", false)
        .expect("Checkout by bookmark name failed");

    // 5. Assert "a.txt" is "v2"
    assert_eq!(fs::read_to_string("a.txt").unwrap(), "v2");
    assert_eq!(
        storage.commits().get_current_head().unwrap().unwrap(),
        hash2
    );
...
fn test_checkout_safety() {
    ...
    assert!(
        result.is_err(),
        "Checkout should fail with unstaged changes"
    );
    ...
    crate::commands::checkout::checkout(storage, &hash1, true)
        .expect("Checkout with force:true should succeed");
