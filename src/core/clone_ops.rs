...
pub fn execute_clone(
    ...
    storage
        .config()
        .set_local("remote.origin.url", &clean_url)?;
    ...
    let (branch, remote_head) = if let Some(b) = specified_branch {
        match client.discover_fetch_refs(&b) {
            Ok(Some(hash)) => (b, hash),
            Ok(None) => {
                return Err(GikError::Branch(format!(
                    "Remote branch '{}' not found. Empty repository?",
                    b
                )))
            }
            Err(e) => return Err(e),
...
pub fn extract_repo_name(url: &str) -> Result<String> {
    ...
    Err(GikError::Branch(
        "Could not extract repository name from URL".to_string(),
    ))
}
...
mod tests {
    ...
    fn test_extract_repo_name() {
        assert_eq!(
            extract_repo_name("https://github.com/Kyrylo/gik.git").unwrap(),
            "gik"
        );
        assert_eq!(
            extract_repo_name("https://github.com/Kyrylo/gik").unwrap(),
            "gik"
        );
        assert_eq!(
            extract_repo_name("https://github.com/Kyrylo/gik/").unwrap(),
