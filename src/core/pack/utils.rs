...
pub fn apply_delta(base: &[u8], delta: &[u8]) -> Result<Vec<u8>> {
    ...
    loop {
        if d_idx >= delta.len() {
            return Err(GikError::Io(std::io::Error::other("Delta truncated")));
        }
        let b = delta[d_idx];
        d_idx += 1;
    ...
    while d_idx < delta.len() {
        ...
        if cmd & 0x80 != 0 {
            ...
            if cmd & 0x01 != 0 {
                offset |= delta[d_idx] as usize;
                d_idx += 1;
            }
            if cmd & 0x02 != 0 {
                offset |= (delta[d_idx] as usize) << 8;
                d_idx += 1;
            }
            if cmd & 0x04 != 0 {
                offset |= (delta[d_idx] as usize) << 16;
                d_idx += 1;
            }
            if cmd & 0x08 != 0 {
                offset |= (delta[d_idx] as usize) << 24;
                d_idx += 1;
            }

            if cmd & 0x10 != 0 {
                size |= delta[d_idx] as usize;
                d_idx += 1;
            }
            if cmd & 0x20 != 0 {
                size |= (delta[d_idx] as usize) << 8;
                d_idx += 1;
            }
            if cmd & 0x40 != 0 {
                size |= (delta[d_idx] as usize) << 16;
                d_idx += 1;
            }
