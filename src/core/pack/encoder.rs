...
pub fn write_object_header<W: Write>(
    mut writer: W,
    obj_type: u8,
    mut size: usize,
    hasher: &mut Sha1,
...
pub fn build_packfile(
    ...
    missing: Vec<crate::core::hash::Hash>,
    ...
    for hash in missing {
        ...
        if let Some(meta) = storage.commits().get_commit_meta(&hash)? {
            ...
            let mut encoder =
                flate2::write::ZlibEncoder::new(temp_pack, flate2::Compression::default());
            encoder
                .write_all(&payload)
            ...
            let (type_str, size, mut payload_stream) =
                crate::core::objects::decompress_object_stream(file)?;
            ...
            let mut encoder =
                flate2::write::ZlibEncoder::new(temp_pack, flate2::Compression::default());
            std::io::copy(&mut payload_stream, &mut encoder)
                .map_err(|e| crate::error::GikError::Io(e))?;
            temp_pack = encoder
                .finish()
                .map_err(|e| crate::error::GikError::Io(e))?;
        } else if let Some(compressed) = storage.objects().get_object(&hash)? {
            let (type_str, size, payload) =
                crate::core::objects::decompress_object(&compressed[..])?;
            let type_id = match type_str.as_str() {
                "tree" => 2u8,
                "blob" => 3u8,
                _ => {
